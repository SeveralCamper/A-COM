...
void printTable(const vector<vector<SimpleFractions>> &matrix,
                vector<int> bazis) {
  cout << setw(WIDTH) << "\\"
       << " |";
...
void printTable(const vector<vector<SimpleFractions>> &matrix,
                vector<int> bazis, set<int> ignoredColumns,
                vector<SimpleFractions> CO, int selectedColumns,
                int selectedRows) {
  cout << setw(WIDTH) << "\\"
       << " |";
  for (int i = 0; i < matrix[0].size(); ++i) {
...
vector<vector<SimpleFractions>>
squareMethod(const vector<vector<SimpleFractions>> &matrix, int row, int col) {
  int n = matrix.size();
  int m = matrix[0].size();
...
vector<vector<SimpleFractions>>
simplexMethod(vector<vector<SimpleFractions>> matrix, vector<int> &basiz,
              vector<bool> fakeCol) {
  int n = matrix.size();
  int m = matrix[0].size();
